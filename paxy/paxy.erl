-module(paxy).
-export([start_acceptor_node/2, start_proposer_node/0, stop/0, stop/1]).
-define(RED, {255,0,0}).
...
start_acceptor_node(Seed, ProposerId) ->
  ...
  receive
    {reqState, State} ->
      ...
      AccRegister2 = lists:map(fun(Name) -> {Name, node()} end, AccRegister),
      ProposerId ! {startprops, PropIds, PropInfo, AccRegister2, Seed}
  end,
  true.
...
start_proposer_node() ->
  ...
  receive
    {startprops, PropIds, PropInfo, AccRegister, Seed} ->
...
start_acceptors(AccIds, AccReg, Seed) ->
    case AccIds of
        ...
        [AccId|Rest] ->
            ...
            AcceptorPid = acceptor:start(RegName, Seed, AccId),
            register(RegName, AcceptorPid),
